...
function insertDataFromFile(table, filePath) {
  console.log(`Starting data insertion for ${table} from ${filePath}`);
  ...
  getTableColumns(table, (err, tableColumns) => {
    ...
    console.log(`Fetched columns for ${table}: ${tableColumns.join(", ")}`);
    ...
    fs.createReadStream(path.resolve(__dirname, filePath))
      ...
      .on("data", async (row) => {
        ...
        connection.query(insertQuery, (err, results) => {
          ...
          console.log(`Inserted data into ${table} successfully: ${JSON.stringify(filteredRow)}`);
        });
      })
      .on("end", () => {
