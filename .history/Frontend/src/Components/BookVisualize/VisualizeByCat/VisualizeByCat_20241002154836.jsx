...
import './VisualizeByCat.css'; // Create a CSS file for styling the component
...
const BookVisualizeByCat = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get("http://localhost:5001/api/borrow/bookVisualizeByCat"); // Adjust the API endpoint if necessary
        const chartData = response.data.map((item, index) => ({
          name: item.Cat_Name, // Assuming your API returns an object with Cat_Name
          value: item.borrowedCount, // Assuming your API returns the count of borrowed books
...
export default BookVisualizeByCat;
