    ...
    const fetchPublisherList = async () => {
        try {
          ...
          const options = response.data.map(publisher => ({
            value: `${publisher.Publisher_ID}. ${publisher.Publisher_First_Name} ${publisher.Publisher_Last_Name}`,
    ...
    const fetchAuthorList = async () => {
        try {
          ...
          const options = response.data.map(author => ({
            value: `${author.Author_ID}. ${author.First_Name} ${author.Last_Name}`,
    ...
    const fetchLanguageList = async () => {
      try {
        ...
        const options = response.data.map(language => ({
          value: `${language.Language_Name}(${language.Language_ID})`,
          label: `${language.Language_Name}(${language.Language_ID})`
        }));
